...
def merge_topics(
    ...
    if len(new_pairs) <= 1 and verbose:
        logger.info("No topic pairs to be merged.")
        ...
        try:
            ...
            for merge in merges:
                ...
                if match:
                    ...
                    logger.info(f"Updated topic tree with [{lvl}] {name}: {desc}")
        except Exception as e:
            logger.info("Error when calling API!")
            traceback.print_exc()
...
def remove_topics(topics_root, verbose, threshold=0.01):
    ...
    for node in topics_root.root.children:
        if node.count < threshold_count and node.lvl == 1:
            node.parent = None
            if verbose:
                logger.info(f"Removing {node.name} ({node.count} counts)")
            removed = True

    if not removed and verbose:
        logger.info("No topics removed.")
...
def replace_topic_key(text, mapping, verbose=False):
    ...
    for key, value in mapping.items():
        if key != value and key in text:
            ...
            if verbose:
                logger.info(f"Replaced '{key}' with '{value}' in text.")
    return text
...
def refine_topics(
    ...
    if verbose:
        logger.info("-------------------")
        logger.info("Initializing topic refinement...")
        logger.info(f"Model: {model}")
        logger.info(f"Input data file: {generation_file}")
    ...
    if mapping_org != mapping and verbose:
        logger.info(f"Mapping updated: {mapping}")
    ...
    logger.info(RenderTree(updated_topics_root.root))
