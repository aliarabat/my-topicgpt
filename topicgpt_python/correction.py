...
def topic_parser(root_topics, df, verbose=False):
    ...
    for i, response in enumerate(df.responses.tolist()):
        ...
        if not extracted_topics:
            if verbose:
                logger.info(f"Error: Row {i} has no topics.")
            error.append(i)
        else:
            for topic in extracted_topics:
                if topic not in valid_topics:
                    if verbose:
                        logger.info(f"Hallucinated: {topic}")
                    hallucinated.append(i)
                    break

    if verbose:
        logger.info(f"Number of errors: {len(error)}")
        logger.info(f"Number of hallucinated topics: {len(hallucinated)}")
...
def correct(
    ...
    for i in tqdm(reprompt_idx, desc="Correcting topics"):
        ...
        try:
            ...
            if verbose:
                logger.info(f"Document {i+1}: {result}")
                logger.info("-" * 20)
            df.at[i, "responses"] = result
        except Exception as e:
            logger.info(f"Error correcting document {i+1}: {e}")
            traceback.print_exc()
...
def correct_batch(
    ...
    for responses, i in zip(responses, reprompt_idx):
        ...
        if verbose:
            logger.info(f"Document {i+1}: {responses}")
            logger.info("-" * 20)
...
def correct_topics(
    ...
    if verbose:
        logger.info("-------------------")
        logger.info("Initializing topic correction...")
        logger.info(f"Model: {model}")
        logger.info(f"Data file: {data_path}")
        logger.info(f"Prompt file: {prompt_path}")
        logger.info(f"Output file: {output_path}")
        logger.info(f"Topic file: {topic_path}")
        logger.info("-------------------")
    ...
    if len(reprompt_idx) > 0:
        ...
        logger.info("All topics are correct.")
        df.to_json(output_path, lines=True, orient="records")
